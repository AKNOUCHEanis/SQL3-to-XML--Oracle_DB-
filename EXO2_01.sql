...
create or replace type body T_Country as
  ...
  where (b.country1= self.code and e.continent=nameContinent and e.country=b.country2)
       or (b.country2= self.code and e.continent=nameContinent and e.country=b.country1);
  ...
  loop
    ...
    output:=XMLType.appendchildxml(output, 'country', XMLType.createxml('<peak height="'|| self.peak()||'"></peak>'));
...
select value(m)
from LesMountains m, LesGeoMountains gm
where m.name=gm.mountain and 'DZ'=gm.country;
